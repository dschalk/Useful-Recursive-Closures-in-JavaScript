...
Object.is(m(dF3x), m_clone(dF3x)): false `;
  ...
  var ex2 = `    var a, b;
    ...
    console.log(b); // [1,2,3]`;
  $$result.css.add(css);
  return `<h1 class="svelte-1cqu3wk" data-svelte-h="svelte-1v2ajup">Simply Cloning Any Object</h1> <h2 class="svelte-1cqu3wk" data-svelte-h="svelte-1o3nol2">Even Deeply Nested Data Structures Containing Functions, Self-References, etc</h2> <pre class="svelte-1cqu3wk">${escape(MCode)}</pre> <p data-svelte-h="svelte-1iuy5qa">Calling m(func) for some function &quot;func&quot; changes x in the m-M(x) closure to func(x) (unless it throws an error); but  has no effect on x in the m_clone-M(x) closure. Likewise, running m_clone(func) does not affect m. Why? <span style="color:coral">Because x gets reassigned on line 4 of M.</span></p> <h2 class="svelte-1cqu3wk" data-svelte-h="svelte-47fcl1">Two Essential Facts</h2> <p data-svelte-h="svelte-91xvdf">1. When two variables point to the same object in memory, either variable can modify that object, thereby changing the value of both variables. For example,
    </p><pre class="svelte-1cqu3wk">${escape(ex1)}</pre> <p data-svelte-h="svelte-1ftglpp">2. When two variables point to the same object in memory, and one gets reassigned  <span style="color:coral">(as happens on line 4 of M)</span>, the variables become independent from one another. For example,
    </p><pre class="svelte-1cqu3wk">${escape(ex2)}</pre> <p data-svelte-h="svelte-1bwnnot">Note: Object.is and === (strict equality) are both used to compare values, but they work differently in two ways: Object.is(NaN, NaN) returns true, whereas NaN === NaN returns false. Object.is(-0, +0) returns true, but -0 === +0 returns false. &quot;===&quot; could have been used in the demonstration below.</p> <pre class="svelte-1cqu3wk">${escape(Obis)}</pre> <p data-svelte-h="svelte-w2xr2d">In the example below, m is defined as an array containing three arrays, one of which contains a function. cl is copy of m. m_clone is a clone of m, m_clone = M(m(dF3x)). The value returned by m_clone(dF3x) is, at first, a reference to the array held in the m-M(x) closure. When m(func) reassigns x in the m-M(x) closure, or m_clone(func) reassigns x in its closure for some function &quot;func&quot;, the value of x in the m-M(x) closure and the value of x in the m_clone-M(x) become independent of one another. These reassignment happen on the 4th line of M.</p> <p data-svelte-h="svelte-1evq3ql">As shown in the following examples, changes in m(dF3x) caused by m(v =&gt; v.concat(888)) don&#39;t change m_clone(dF3x), and changing m_clone(dF3x) by running m_clone(v =&gt; v.concat(777)) has no effect on m or cl. Unlike m and m_clone, m and cl point to the same place in memory, as do the values held in the m-M(x) and cl-M(x) closures; i.e., m(dF3x) and cl(dF3x). Therefore, changing x in either closure changes x in the other.</p> <p data-svelte-h="svelte-1ffoxs1">Here&#39;s the demonstration code:</p> <pre class="svelte-1cqu3wk">${escape(code)}</pre> <br> <p data-svelte-h="svelte-190rqbo">This is the text returned by the demonstration code using Node.js:</p> <pre class="svelte-1cqu3wk">${escape(output)}</pre>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte2-CGhGAolw.js.map
