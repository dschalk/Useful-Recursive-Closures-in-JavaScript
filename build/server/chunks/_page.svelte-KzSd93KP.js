...
percent deviation from equality is 0.000019202943251461886 %`;
  $$result.css.add(css);
  return `<div style="width: 80%; margin-left: 10%"><h1 style="text-align:center" data-svelte-h="svelte-sjw85c">Martingale Betting Strategy</h1> <p data-svelte-h="svelte-mb9knk">&quot;Instead of traditional recursion, where a function calls itself directly and builds up a call stack, you&#39;re calling m(f1) in a way that resets the closure’s state rather than creating new stack frames.&quot; -- ChatGPT</p> <p data-svelte-h="svelte-1gsbrdz"><a href="https://medium.com/@pelicanlabs/a-curious-conclusion-from-the-martingale-betting-strategy-28dc297a5fee">The Martingale &quot;Double Your Money&quot; Betting Strategy</a> is, essentially, a system in which players double their bets each time they lose in an even-odds game of chance. Examples are calling &quot;heads&quot; or &quot;tails&quot; on fair flips of a fair coin, and betting on red or black on spins of a roulette wheel.</p> <p data-svelte-h="svelte-v5s50j">When &quot;Play&quot; is clicked (below), line 3 of the function &quot;f1&quot; returns either 0 or 1 with equal probability. These numbers are equivalent to false and true in the test on line 4 (&quot;if (result) ...&quot;).</p> <p data-svelte-h="svelte-pwf1s1">Clicing &quot;Play&quot; calls m(f1), where m = M([25,1,25,0]) and x is [25,1,25,0] in the resulting <a href="./">m-M(x) closure</a>. &#39;x&#39; represents [starting amount, current bet, goal, number of wins], (beginning with [25,1,25,0]).</p> <button data-svelte-h="svelte-1bwigta">Play</button> <button data-svelte-h="svelte-1k8q8lq">Clear</button> <div><h2 class="svelte-6teu96" data-svelte-h="svelte-ywgowz">GAME OUTPUT</h2> <pre>${escape(output)}</pre>  <p data-svelte-h="svelte-15yhb88">&quot;********************************************&quot;</p></div> <p data-svelte-h="svelte-hou68t">Players start with N dollars (m(dF3x)[0] === N), and stop if m(dF3x)[0] reaches 2N. Otherwise, players stop when they wouldn&#39;t have enough money to cover losing the next bet; i.e., when m(dF3x)[0] is less than m(dF3x)[1]. Here&#39;s the code:</p> <pre>${escape(stdCode)}</pre> <p data-svelte-h="svelte-ix6wr3">Each time a player loses, the amount of the bet (which starts at $1) doubles. If the player wins a bet, all prior losses are recouped and $1 is gained. For example, if the bets were $1, $2, $4,and $8, winning $16 recoups the $(1+2+4+8) = $15, leaving one extra dollar.</p> <p data-svelte-h="svelte-oaebf5">When m(dF3x)[3] === m(dF3x)[2], the player&#39;s money has doubled because each incremental increase in m(dF3x)[3] corresponds to an incremental increase in m(dF3x)[0]. The final value of x in the m-M(x) closure is m(dF3x) === [50,1,25,25].</p> <p data-svelte-h="svelte-1iuwrhm">If v[3] is less than the starting amount, f1 calls m on itself to initiate another round of action. The semi-recursive function &quot;f1&quot; -- f1 repeatedly calls m(f1), causing f1 to execute on x in the closure --  executes on x until m(dF3x)[2] equals m(dF3x)[3] (Success), or m(dF3x)[0] is less than m(dF3x)[1] (Fail).</p> <p data-svelte-h="svelte-igd3es">Players fail most attempts to double their money. Lacking sufficient funds to cover a losing bet, they leave with whatever they still have. The amount they lose varies, but it&#39;s rarely everything they started with.</p> <h2 class="svelte-6teu96" data-svelte-h="svelte-ztncb0">A BILLION ATTEMPTS TO DOUBLE $50 IN AN m-M(x) CLOSURE</h2> <p data-svelte-h="svelte-149l718">A file named test13.js is shown below. Entering &quot;node test13&quot; in my Linux desktop computer&#39;s simulated terminal starts a process that takes about 25 minutes to complete.  It uses only 6.2% of the available CPU potential, and a miniscule 60 megabytes of the 64 gigabytes of installed memory. There&#39;s obviously no danger of piling too many frames onto the stack.</p> <p data-svelte-h="svelte-179ik1j">Here&#39;s test13.js:</p> <pre>${escape(gambleCode)}</pre> <p style="margin: 3%" data-svelte-h="svelte-1qkwhvl">The result of calling gamble (above) five consecutive times, each time trying to double $50 a billion times, strongly suggests that repeated use of the Martingale betting strategy is a 50-50, break even proposition, similar to betting the same amount on coin flips thousands of times. I don&#39;t know how to prove it, but it seems self-evident that no pattern of changing the amounts of bets can possibly increase or decrease the odds of coming out ahead after repeatedly betting on fair flips of a fair coin.</p> <p data-svelte-h="svelte-1jbmj7f">Here are the results:</p> <pre>${escape(runs)}</pre> <br><br><br></div>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-KzSd93KP.js.map
